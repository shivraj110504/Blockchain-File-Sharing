...
app.secret_key = os.environ.get("SECRET_KEY", "storiumx_default_secret_key_change_me")
...
# MongoDB Connection
MONGODB_URI = os.environ.get("MONGODB_URI", "mongodb://localhost:27017/storiumx")
client = MongoClient(MONGODB_URI)
db = client.get_database()  # Use database from connection string
users_col = db["users"]
files_col = db["files"]
...
def signup():
    ...
    # Generate unique key for blockchain operations
    ...
    flash(f"Welcome to StoriumX, {name}!")
    print(f"DEBUG: New user created - Name: {name}, Email: {email}, Key: {user_key}")
    return redirect("/")
...
def login():
    ...
    if bcrypt.checkpw(password.encode('utf-8'), user["password_hash"]):
        ...
        print(f"DEBUG: User logged in - Name: {user['name']}, Email: {email}")
        return redirect("/")
    else:
        flash("Invalid email or password")
        return redirect("/")
...
def register():
    """Legacy route - redirect to signup"""
    return signup()
